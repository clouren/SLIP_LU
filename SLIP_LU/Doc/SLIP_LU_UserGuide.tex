...
\vspace{5mm}
Version 1.0.0, May 2020 % VERSION
\vspace{20mm}
...
\verb|SLIP_LU.m| function. First, this section describes the \verb|options|
...
The \verb|SLIP_LU.m| function solves the linear system $A x = b$ where $A \in
\mathtt{R}^{n \times n}$, $x \in \mathtt{R}^{n \times m}$ and $b \in
...
A future version of the \verb|SLIP_LU| MATLAB function could return \verb|x|
in the MATLAB \verb|vpa| format, which would preserve the exact solution.
