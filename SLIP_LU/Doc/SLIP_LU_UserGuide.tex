...
the solution vector(s) output in double precision are accurate to machine
...
\begin{itemize}
    ...
    stored in the sparse CSC form with \verb|mpz_t| entries.
...
created object.  The object contains an array \verb|S->q| of length \verb|n+1|
where \verb|n| is the number of columns in the matrix to be analyzed.  Returns
\verb|NULL| on failure.  Both \verb|S->lnz| and \verb|S->unz| are set to 0. For
more information about the \verb|SLIP_LU_analysis| structure, users can refer
to Section \ref{ss:SLIP_LU_analysis}.
...
% TODO what is "initialzed but unused" ???
...
{\bf TODO: why a leak???} % TODO I don't understand this.
...
%-------------------------------------------------------------------------------
\subsection{Utility Routines (TODO rename this)}
% TODO these are not "Utility Routines"
%-------------------------------------------------------------------------------
...
is done via $n$ (number of rows and columns of $A$) iterations of the sparse
...
please refer to either \verb|Demos/SLIPLU.c| or Section \ref{s:Using:hard}.
...
\verb|p[i]| gives the row index in $PA$).
...
% TODO: we cannot say this function is "primary internal". 
Upon completion, \verb|x| contains the solution to the \textit{scaled} linear
system. Like some of the other routines discussed in this section, this
function is primarily internal; thus for usage information please refer to
either \verb|Demos/SLIPLU.c| or Section \ref{s:Using:hard}.
...
% TODO: we cannot say this function is "primary internal". 
Like some of the other routines discussed in this section, thus function is
primarily internal; thus for usage information please refer to either
\verb|Demos/SLIPLU.c| or Section \ref{s:Using:hard}.
...
% TODO why is b sometimes $b$ and sometimes $\mathbf{b}$?
% TODO: we cannot say this function is "primary internal". 
\verb|SLIP_scale_x| scales solution vector with scaling factors of $A$ and $b$.
SLIP LU will scale the user's input matrix to ensure everything is integer;
thus, once the rational solution vector \verb|x| is obtained, it must be
properly scaled so that it is accurate. Again, this is mainly an internal
function, thus for usage, please refer to either \verb|Demos/SLIPLU.c|
or Section \ref{s:Using:hard}.
...
returns the solution as a matrix accurate to double precision. This is a
"all-in-one" function that performs factorization, solving, permutation and
scaling.
% TODO: so it is not all-in-one:
However, symbolic analysis \verb|SLIP_LU_analysis| should be performed
before calling this function.

On output, this \verb|x_doub| contains the solution to the linear system in
double precision and the function returns \verb|SLIP_OK|.

For the full example, users can refer to \verb|Demos/example3.c|. Here is an
...
returns the solution as a matrix of \verb|mpq_t| numbers. This is a
"all-in-one" function that performs factorization, solving, permutation and
scaling.
% TODO: so it is not all-in-one:
However, symbolic analysis \verb|SLIP_LU_analysis| should be performed
...
On output, this \verb|x_mpq| contains the exact solution to the linear system
as \verb|mpq_t| numbers and the function returns \verb|SLIP_OK|

For the full example, users can refer to \verb|Demos/example2.c|. Here is an
brief example of how to use this code:
...
\verb|SLIP_solve_mpq| solves the linear system $A\mathbf{x}=\mathbf{b}$ and
returns the solution as a matrix of \verb|mpfr_t| numbers. This is a
"all-in-one" function that performs factorization, solving, permutation and
scaling.
% TODO: so it is not all-in-one:
However, symbolic analysis \verb|SLIP_LU_analysis| should be performed
...
On output, this \verb|x_mpfr| contains the exact solution to the linear system
...
% TODO: m and n must be > 0, right?  Say so in each function.
...
entry. $A[i]$ is a pointer to row $i$, of size $n$.
...
associated with each entry is given by \verb|option->prec|.
...
% TODO why say this?
When using the GMP library for \verb|SLIP LU|, it is highly recommended that
the user uses the defined \verb|SLIP_delete*| functions in order to avoid
memory leaks or potential segmentation faults.
...
%-------------------------------------------------------------------------------
\subsection{Miscellaneous Routines2 (TODO rename this)}
%-------------------------------------------------------------------------------

TODO:  Yikes, and I thought the section ``Miscellaneous Routines'' was vague.
Fix this.
...
This function reads in an integer matrix stored in
% TODO this function des *NOT* read Matrix Market format.  Describe the format.
matrix market format from the file \verb|filename|. On success, the value
\verb|SLIP_OK| is returned. The following code snippet shows how to use this
function.
...
This function assumes \verb|b| is $n \times 1$ and reads in the integer right
hand side vector from the file \verb|filename|.

%-------------------------------------------------------------------------------
\subsection{SLIP LU GMP and MPFR Wrappers}
%-------------------------------------------------------------------------------

As discussed previously (TODO: where?  Give the section number), SLIP LU
provides a wrapper class for all GMP and MPFR functions used by SLIP LU.
The wrapper class provides error-handling for out-of-memory conditions that
are not handled by the GMP and MPFR libraries.
...
\verb|TYPEa| and \verb|TYPEb| can be GMP type data (e.g., \verb|mpz_t|,
\verb|mpq_t| and \verb|mpfr_t|) or non-GMP type data (e.g., \verb|int|,
\verb|double|), and they need not to be the same. In order to apply our wrapper
to a new function, one can create it as follows:
...
% TODO: most should??  THis is ambiguous.
Note that, most of the wrapped GMP/MPFR functions (other than the \verb|print|
functions) should always return \verb|SLIP_info| to the caller. Therefore, for
some GMP/MPFR functions that have their own return value, e.g.,
\verb|int mpq_cmp (const mpq_t a, const mpq_t b)|, the return value need to be
added into wrapped function. In general, a GMP/MPFR function in the form of
\verb|TYPEr gmpfunc(TYPEa a, TYPEb b, ...)|, user can create the wrapped
function as follows:
...
\verb|mpz_init2(x, size)|
    & \verb|SLIP_info SLIP_mpz_init2(x, size)|
...
for this purpose which is discussed in Section \ref{s:Using:easy}.
Alternatively, if the user wants the actual $L$ and $U$ factors, please refer
to Section \ref{s:Using:hard}. Finally, step 4 is discussed in Section
...
%-------------------------------------------------------------------------------
\cprotect\subsection{Easy SLIP LU for Solving Linear Systems (no L and U)}
\label{s:Using:easy}
%-------------------------------------------------------------------------------
...
the ``easy'' interface of SLIP LU requires only that the user decides what data
...
%-------------------------------------------------------------------------------
\cprotect\subsection{Complex SLIP LU for Solving Linear Systems (with L and U)}
\label{s:Using:hard}
%-------------------------------------------------------------------------------

% TODO: Complex?  As in real and imaginary parts?  The use of the word
...
freed with a call to the macro \verb|SLIP_FREE()|, e.g.,
...
% TODO the input file is not Matrix Market format
\item \verb|example2.c|: This example reads in a matrix stored in integer
matrix market format from the ExampleMats folder. Additionally, it reads in a
...
using the techniques discussed in Section \ref{s:Using:hard}. This file also
