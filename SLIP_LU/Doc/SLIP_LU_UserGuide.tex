...
% TODO: This is not the set of enums that SLIP_get_options.m says.
% It says 0:none, 1:colamd, 2:amd.  FIXME.
\item \verb|option.order|: This parameter controls the column ordering used. 0
...
% TODO why have A_is_integral and b_is_integral anyway?  Can the function
% check this itself?

\item \verb|option.A_is_integral|: Set this parameter equal to 1 if the input
matrix \verb|A| is already integral.
% TODO do not say "integral".  Elsewhere in SLIP_LU the term is used for mpz_t.
Otherwise, if the input matrix has any
decimal entries, scaling must be performed to obtain an integral input matrix.
\import If the input matrix is not integral and this parameter is set equal to
1, the values will be truncated.
% TODO truncated???  No.  Typecasted to int32_t, which may be undefined if the
% entries in A are outside the range -2^31 to 2^31.

% TODO: what does "integral" mean?  slip_mex_get_A_and_b assumes the entries in
% A can be int32.  So "integral" is too strong.  Should be "All entries in A
% are integers in the range -2^31 to 2^31."  Likewise for b.
