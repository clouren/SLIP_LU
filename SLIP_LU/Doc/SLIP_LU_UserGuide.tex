...
% TODO: we cannot say this function is "primary internal". JC fixed 
...
primarily for advanced users who might want intermediate calculation results;
thus for usage information please refer to either \verb|Demos/SLIPLU.c| or
Section \ref{s:Using:hard}.
...
% TODO why is b sometimes $b$ and sometimes $\mathbf{b}$? JC fixed
% TODO: we cannot say this function is "primary internal". JC fixed
...
returns the solution as a matrix accurate to \verb|double| precision. This is a
...
% TODO why say this?
% JC: since for mpfr/mpq/mpz array/mat, we need to free each entry individually
...
\begin{comment}%delete this section since these are function in Demo
%-------------------------------------------------------------------------------
...
As briefly mentioned in Section \ref{s:intro}(TODO: where?  Give the section number), SLIP LU
...
% TODO: most should??  THis is ambiguous. JC fixed
...
GMP/MPFR functions that have their own return value, e.g.,
...
\verb|TYPEr gmpfunc(TYPEa a, TYPEb b, ...)|, users can create the wrapped
function as follows:
...
% TODO o.w. is confusing. JC fixed
...
% "Complex" is confusing. JC: advanced?
...
% TODO the input file is not Matrix Market format JC: changed to CSC?
\item \verb|example2.c|: This example reads in a matrix stored in integer
CSC format from the ExampleMats folder. Additionally, it reads in a
