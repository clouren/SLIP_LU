...
TODO: fixme; describe the \verb'SLIP_matrix'. % TODO
...
\verb|A->i|. Note that \verb|A->nzmax| $\geq$ \verb|nnz(A)|, where
\verb'nnz(A)' is the return value of \verb'SLIP_matrix_nnz(A,option)'.
Internally, this parameter serves as an estimate on the amount of memory needed
and is used to reduce the number of intermediate reallocations performed in the
library. Data Type: \verb|int64_t|

\item \verb|A->p|: An array of size $n+1$ which contains column pointers of
$A$. Data Type: \verb|int64_t*|
...
\verb|SLIP_build_sparse_csc_int64|
    & build sparse matrix from \verb|int64_t| type CSC matrix
    & \ref{s:user:build_sparse_csc_int64} \\
...
\verb|SLIP_build_sparse_trip_int64|
    & build sparse matrix from \verb|int64_t| type triplet-format matrix
    & \ref{s:user:build_sparse_trip_int64} \\
...
\begin{itemize}
\item \verb|b->m|: Number of rows in the matrix. Data Type: \verb|int64_t|
\item \verb|b->n|: Number of columns in the matrix. Data Type: \verb|int64_t|
...
\verb|SLIP_build_dense_int64|
    & build \verb'SLIP_dense' matrix from a 2D \verb|int64_t| array
    & \ref{s:user:build_dense_int64} \\
...
%-------------------------------------------------------------------------------
\cprotect\subsubsection{\verb|SLIP_build_sparse_csc_int64|: from \verb|int64_t| entries}
\label{s:user:build_sparse_csc_int64}
%-------------------------------------------------------------------------------
...
\verb|SLIP_build_sparse_csc_int64| builds a sparse matrix using compressed column
form inputs, where the entry values are \verb|int64_t| type.
...
\begin{verbatim}
    ...
    (
        ...
        int64_t n,          // dimension of the matrix
        int64_t nz          // number of nonzeros in A (size of x, I, and J)
    ) ;
\end{verbatim}
} \end{mdframed}
...
%-------------------------------------------------------------------------------
\cprotect\subsubsection{\verb|SLIP_build_sparse_trip_int64|:
from \verb|int64_t| entries}
\label{s:user:build_sparse_trip_int64}
%-------------------------------------------------------------------------------
...
\begin{verbatim}
    ...
    (
        ...
        int64_t n,          // dimension of the matrix
        int64_t nz          // number of nonzeros in A (size of x, I,
                            // and J vectors)
    ) ;
\end{verbatim}
} \end{mdframed}

\verb|SLIP_build_sparse_trip_int64| builds a sparse matrix from triplet form
inputs, where the entry values are \verb|int64_t| type.
...
%-------------------------------------------------------------------------------
\cprotect\subsubsection{\verb|SLIP_build_dense_int64|: from \verb|int64_t| array}
\label{s:user:build_dense_int64}
%-------------------------------------------------------------------------------
...
\verb|SLIP_build_dense_int64| builds a \verb'SLIP_dense' matrix \verb'C'
from a 2D \verb|int64_t| array \verb'B'.
...
%-------------------------------------------------------------------------------
\cprotect\subsection{\verb|SLIP_create_int64_mat|: create a $m$-by-$n$ \verb|int64_t| matrix}
%-------------------------------------------------------------------------------
...
\verb|SLIP_create_int64_mat| allocates a \verb|int64_t| matrix of size $m \times
...
%-------------------------------------------------------------------------------
\cprotect\subsection{\verb|SLIP_delete_int64_mat|: delete a $m$-by-$n$ \verb|int64_t| matrix}
%-------------------------------------------------------------------------------
...
\verb|SLIP_delete_int64_mat| frees the memory associated with a \verb|int64_t|
matrix of size $m \times n$, and sets \verb|**A=NULL|.
...
\begin{verbatim}
    ...
    (
        int64_t n,     // size of the array (must be > 0)
        SLIP_options *option  // command options containing the prec for mpfr
    ) ;
\end{verbatim}
} \end{mdframed}
...
one of 5 options: \verb|int64_t|, \verb|double|, \verb|mpfr_t|, \verb|mpq_t|,
...
\begin{verbatim}
    ...
    /* Get the matrix A. Assume that everything is stored in
       compressed column form. This means that int64_t* I is the
       set of row indices, int64_t* p are the column pointers, x
    ...
    {
        SLIP_build_sparse_csc_int64(&A, p, I, x, n, nz) ;
    }
    else if (X IS mpq_t)
    {
...
\begin{verbatim}
    /* Get the matrix A. Assume that everything is stored in
       compressed column form. This means that int64_t* I is the
       set of row indices, int64_t* J is the set of column indices,
    ...
    {
        SLIP_build_sparse_trip_int64(&A, I, J, x, n, nz) ;
    }
    else if (X IS mpq_t)
    {
...
{\small
    ...
    else if (b2 IS int64_t)
    {
        SLIP_build_dense_int64(&b, b2, n, numRHS) ;
    }
    else if (b2 IS mpq_t)
    {
...
\begin{verbatim}
    if (USER WANTS MPQ)
    {
        ...
        SLIP_info info = SLIP_solve_mpq(soln, A, S, b, option) ;
    }
    else if (USER WANTS DOUBLE)
    {
        // The solution is a dense matrix of size n*numRHS
        double** soln = SLIP_create_double_mat(n, numRHS) ;
        SLIP_info info = SLIP_solve_double(soln, A, S, b, option) ;
    }
    else if (USER WANTS MPFR)
    {
        // The solution is a dense matrix of size n*numRHS
        mpfr_t** soln = SLIP_create_mpfr_mat(n, numRHS, option) ;
        SLIP_info info = SLIP_solve_mpfr(soln, A, S, b, option) ;
    } \end{verbatim} }
