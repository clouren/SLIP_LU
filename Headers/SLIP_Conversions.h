...
/* Purpose: This function transposes the matrix A and stores it in matrix B
   Arguments:
   A: matrix to be transposed
   B: matrix which stores A' */
void SLIP_transpose (SLIP_mat *A, SLIP_mat *B)
{
	int i, q, j, n;
	n = A->n;
	int* w = SLIP_initialize_int_array(n);
	SLIP_mat_alloc(n, n, A->nz, B);			    // Allocate B
	for (i = 0; i < A->p[n]; i++) w[A->i[i]]++;	// Set column pointers of B
	cs_cumsum(B->p, w, n);				        // Set B->p
	for (j = 0; j < n; j++)				        // Set numeric values of B
	{
...
/* Purpose: compute x = x+beta*A(:,j)
   Arguments:
   A: matrix extracting col j from
   j: column of A to extract
   beta: scale of A
   w: workspace vector
   x: solution vector
   mark: where to stop in C
   C: Storing solution
   nz: number of nonzeros in product */
int cs_scatter (SLIP_mat *A, int j, mpz_t beta, int *w, mpz_t *x, int mark, SLIP_mat *C, int nz)
{
	int i, p;					                // Initialize memory
	for (p = A->p[j]; p < A->p[j+1]; p++)		// Iterate across nonzeros in column j
	{
		i = A->i[p];
		if (w[i] < mark)
		{
...
/* Purpose: This functions compute C = A*B
   Arguments:
   A: first matrix
   B: second matrix
   C: C = A*B */
void SLIP_multiply (SLIP_mat* A, SLIP_mat* B, SLIP_mat *C)
{
	int n, j, nz = 0, p, i;
	mpz_t* x = SLIP_initialize_mpz_array(n);				// Initialize memory
	n = A->n;
	int* w = SLIP_initialize_int_array(n);
	SLIP_mat_alloc(n, n, A->nzmax + B->nzmax, C);			// Allocate size for a m*n C matrix
	C->nz = C->nzmax;								        // Set number of nonzeros for now
	for (j = 0 ; j < n ; j++)
	{
		if (nz + n > C->nzmax)							    // Reallocate C if necessary
		{
			SLIP_mat_realloc(C);
		}
		C->p[j] = nz;								        // Finalize C->p[j]
		for (p = B->p[j]; p < B->p[j+1]; p++)				// Get column j
			nz = cs_scatter (A, B->i[p], B->x[p], w, x, j+1, C, nz) ;
		for (p = C->p[j] ; p < nz ; p++) mpz_set(C->x [p], x [C->i [p]]);	// Set numeric values
	}
	C->p[n] = nz;									        // Finalize C
	C->nz = nz;
	SLIP_mat_collapse(C);								    // Free memory from C
}
