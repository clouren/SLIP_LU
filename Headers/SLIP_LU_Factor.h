...
/*----------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------
----------------Sparse SLIP Cholesky------------------------------------------------------------------------------------
NEW STUFF THIS HAS NOT BEEN SHOWN TO TIM
------------------------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------------*/

/* p [0..n] = cumulative sum of c [0..n-1], and then copy p [0..n-1] into c 
   From Tim Davis SuiteSparse */
double cs_cumsum_chol (int *p, int *c, int n)
{
	int i, nz = 0 ;
	double nz2 = 0 ;
...
SLIP_mat* SLIP_transpose_chol (SLIP_mat *A)
{
...
/* Purpose: This function performs the sparse REF triangular solve for a cholesky input matrix A(p,p)
   It's essentially a simplified version of the general REF triangular solve.
   Arguments:
   L: partial L matrix
   A: input matrix
   k: iteration of algorithm
   xi: nonzero pattern vector
   q: column permutation (REMOVED)
   rhos: sequence of pivots
   pinv: inverse row permutation 
   row_perm: row permutation (REMOVED)
   col_loc: column permutation (REMOVED)
   h: history vector
   x: solution of system ==> kth column of L and U */
int SLIP_chol_triangular_solve(SLIP_mat* L, SLIP_mat* A, int k, int* xi, mpz_t* rhos, int* pinv, int* h, mpz_t* x)
{
	int j, i, p, m, top, n;
	/* Initialize REF TS by getting nonzero patern of x && obtaining A(:,k) */
	n = A->n;												                // Size of matrix and the dense vectors
	//col = q[k];												                // Column we are solving for
	top = cs_reach(L, A, k, xi, pinv);										// Obtain nonzero pattern stored in xi[top..n]
	SLIP_sort_xi(xi, top, n, pinv, pinv);								// Sort xi wrt sequence of pivots
	//std::cout<<"\nxi is: \n";
	//for (i = 0; i < n; i++) std::cout<< xi[i] << " ";
	SLIP_reset_mpz_array_2(x,n,top,xi);									    // Reset x[i] = 0 for all i in nonzero pattern
	SLIP_reset_int_array_2(h,n,top,xi);									    // Reset h[i] = -1 for all i in nonzero pattern
	SLIP_get_column(A,k,x);										        // Set x = A(:,k)
	
	//SLIP_mat* U = SLIP_transpose_chol(L);
	for (j = 0; j < k; j++)
	{
...
// 	SLIP_mat_delete(U);
	...
	{   
		...
		if (j < k)											            // jnew < k --> entries in U
		{
			...
			/* IPGE updates */
			//col = col_loc[q[j]];									        // Corresponding column location of this x
			for (m = L->p[j]; m < L->p[j+1]; m++)						// Iterate accross nonzeros in Lij
			{
				i = L->i[m];									            // i value of Lij
				if (i > j && i >= k)
				{
					...
					if (mpz_sgn(x[i]) == 0)							        // lij is nonzero, x[i] is zero
					{
						...
						{
							mpz_submul(x[i],L->x[m],x[j]);				    // x[i] = 0 - lij*x[j]
							mpz_divexact(x[i],x[i],rhos[j-1]);			// x[i] = x[i] / rho[j-1]
							h[i] = j;						            // Entry is up to date
						}
					}
					else									                // Both lij and x[i] are nonzero
					{
						/* x[i] != 0 --> History & IPGE update on x[i] */
						if (j < 1)							            // No previous pivot in this case
						{
							mpz_mul(x[i],x[i],rhos[0]);				        // x[i] = x[i]*rho[0]
							mpz_submul(x[i], L->x[m], x[j]);			    // x[i] = x[i] - lij*xj
						...
						{
							if (h[i] < j - 1)					        // History update if necessary
							{
								mpz_mul(x[i],x[i],rhos[j-1]);		    // x[i] = x[i] * rho[j-1]
								if (h[i] > -1)
									mpz_divexact(x[i],x[i],rhos[h[i]]);	    // x[i] = x[i] / rho[h[i]]
							}
							mpz_mul(x[i],x[i],rhos[j]);				    // x[i] = x[i] * rho[j]
							mpz_submul(x[i], L->x[m], x[j]);			    // x[i] = x[i] - lij*xj
							mpz_divexact(x[i],x[i],rhos[j-1]);			// x[i] = x[i] / rho[j-1] 
							h[i] = j;						            // Entry is up to date
						}
...
/* Purpose: This function performs the SLIP Cholesky factorization. This factorization is done via n iterations of the
   sparse REF triangular solve function. The overall factorization is PAP = LDL'
   Arguments:
   A: matrix to be factored
   L: lower triangular matrix (REMOVED)
   U: upper triangular matrix 
   S: stores guess on nnz and column permutation
   rhos: sequence of pivots
   pinv: inverse row permutation
   option: command options */
int SLIP_Chol(SLIP_mat* A, SLIP_mat* L, SLIP_col* S, mpz_t* rhos, int* pinv, SLIP_LU_Options* option)
{
	std::chrono::steady_clock::time_point t_begin = std::chrono::steady_clock::now();	// Begin timing factorization
	int n, numRealloc = 0, k = 0, top, i, j, col, loc, lnz = 0, unz = 0, pivot, check, jnew, *xi, *h, *col_loc, *pivs, *row_perm;
	long size;
	...
	//S->lnz = 10;
	//std::cout<<"\nS->lnz is: "<<S->lnz;
    SLIP_mat_alloc2(n, n, S->lnz, L);			            // Allocate L 
    
	/* Iteration 0, must select pivot */
	SLIP_get_column(A,0,x);				               		 // x = A(:,0)
	if ( mpz_sgn(x[0]) != 0 )			// Check if x[col] is eligible
	{
		pivot = 0;
		mpz_set(rhos[0], x[0]);
	}
		...
		pivot = -1;
	//std::cout<<"\nA->p[1] is: "<<A->p[1];
	top = n-A->p[1]; j = 0;			        				// top: nnz in column 0
	for (i = A->p[0]; i < A->p[1]; i++)		     		   // Populate nonzero pattern
	{
		xi[top+j] = A->i[i];
		j+=1;
	}
	if (pivot == -1) 					                    // The matrix is not SPD
	...
	for (j = top; j < n; j++)				                // Populate L and U
	{
		...
		if (jnew >= 0)					                    // L entries
		{
			...
			size = mpz_sizeinbase(x[jnew],2);	            // Allocate memory for x[j]
			mpz_init2(L->x[lnz],size+2);                    // GMP manual: Allocated size should be size+2
			mpz_set(L->x[lnz],x[jnew]);                     // Set U[x]
			lnz += 1;                                       // Increment nnz of U
		}
	}
	...
	for (k = 1; k < n; k++)
	{
		...
		//	numRealloc +=1;
		//}
		top = SLIP_chol_triangular_solve(L, A, k, xi, rhos, pinv, h, x);
		if ( mpz_sgn(x[k]) != 0 )			// Check if x[col] is eligible
		{
			pivot = k;
			mpz_set(rhos[k], x[k]);
		}
		...
		if (pivot < 0) 							            // Error
		{
			std::cout<<"\n\n**ERROR** No pivot at iteration "<<k<<"! Matrix is not SPD!\n\n";
			return -1;
		}
		for (j = top; j < n; j++) 					        // Iterate accross the nonzeros in x
		{
			jnew = xi[j];
			if (jnew >= k)						            // loc <= k are rows above k, thus go to U
			{
				L->i[lnz] = jnew;                           // Place the i location of the U->nz nonzero
				size = mpz_sizeinbase(x[jnew],2);
				mpz_init2(L->x[lnz],size+2);                // GMP manual: Allocated size should be size+2
                mpz_set(L->x[lnz],x[jnew]);                 // Place the x value of the U->nz nonzero
                lnz += 1;                                   // Increment U->nz
			}
		}
	}
	L->nz = lnz;
...
/* Purpose: This function solves the linear system LD^(-1)U x = b.
   ...
   option: command options
   numRHS: number of RHS vectors */
void SLIP_Chol_Solve(mpq_t** x, mpz_t** b, mpz_t* rhos, SLIP_mat* L, SLIP_mat* U, int* pinv, SLIP_LU_Options* option, int numRHS)
{
	std::chrono::steady_clock::time_point t_s_begin = std::chrono::steady_clock::now();
	int i, k, n = L->n;
	mpz_t** b2 = SLIP_initialize_mpz_mat(n, numRHS);	    // Permuted b
